...
export function Reminders({ reminders, setReminders, addToast }) {
 ...
 const enableNotifications=async()=>{if(typeof Notification==='undefined')return addToast('Notifications are not supported by this browser.'); const result=await Notification.requestPermission();setPermission(result);addToast(result==='granted'?'Alarm notifications enabled.':'Please allow notifications in your browser settings.')}
 useEffect(()=>{const check=()=>{if(typeof Notification==='undefined'||Notification.permission!=='granted')return;const now=new Date(),time=now.toTimeString().slice(0,5);reminders.filter(r=>r.date==='Today'&&!r.complete&&r.time===time&&!alerted.current.has(r.id)).forEach(r=>{alerted.current.add(r.id);new Notification('Medkit medicine alarm',{body:`Time to take ${r.medicine}.`,icon:'/favicon.ico'});addToast(`Alarm: time to take ${r.medicine}`)})};check();const timer=window.setInterval(check,15000);return()=>window.clearInterval(timer)},[reminders,addToast])
 const add=e=>{e.preventDefault();if(!draft.medicine.trim())return;setReminders([...reminders,{...draft,id:Date.now(),complete:false}]);setModal(false);addToast('Alarm reminder created')}; const toggle=id=>setReminders(reminders.map(r=>r.id===id?{...r,complete:!r.complete}:r)); const remove=id=>{setReminders(reminders.filter(r=>r.id!==id));addToast('Reminder deleted')}; const next=reminders.find(r=>r.date==='Today'&&!r.complete)
