...
export default function App() {
  const location = useLocation(),
    ...
    [reminders, setReminders] = useState(initialReminders),
    [toast, setToast] = useState("");
  const appRoute = !["/", "/login", "/register", "/about"].includes(
    location.pathname,
  );
  const addToast = (text) => {
    setToast(text);
      ...
      {appRoute ? (
        <AppShell open={menu} onClose={() => setMenu(false)}>
          <Routes>
            <Route
              path="/dashboard"
              element={<Dashboard reminders={reminders} />}
            />
            <Route path="/medicines" element={<Medicines />} />
            <Route
              path="/medicines/:id"
              element={<MedicineDetail addToast={addToast} />}
            />
